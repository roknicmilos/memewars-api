...
class TestMemeAdmin(TestCase):
    ...
    def test_should_declared_readonly_fields_when_adding_meme(self):
        actual_readonly_fields = self.meme_admin.get_readonly_fields(request=self.request_example)
    ...
    def test_should_declared_and_additional_readonly_fields_when_adding_meme(self):
        meme = MemeFactory()
        actual_readonly_fields = self.meme_admin.get_readonly_fields(request=self.request_example, obj=meme)
