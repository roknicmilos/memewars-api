...
from meme_wars.utils import reverse_admin
...
class TestUserSettingsAdmin(TestCase):
    ...
    def test_should_redirect_from_list_view_to_details_view(self):
        ...
        list_url_path = reverse_admin(
            view_name=f'admin:{self.user_settings_app_label}_{self.user_settings_model}_changelist'
        )
        self.create_and_login_superuser()
        response = self.admin_client.get(path=list_url_path)
    ...
    def test_should_create_user_settings_when_accessing_details_view_and_user_settings_do_not_exist(self):
        self.assertFalse(UserSettings.exists())
        self.create_and_login_superuser()
        details_url_path = reverse_admin(
            view_name=f'admin:{self.user_settings_app_label}_{self.user_settings_model}_change',
            args=(1,)
        )
        response = self.admin_client.get(path=details_url_path)
        self.assertTrue(UserSettings.exists())
